...
ok(kits.length === 5, `les 5 kits existants sont récupérés dans les données [${kits.length}]`);
const cartes = await page.locator('.kit-card').count();
ok(cartes === 5, `« Mes kits » affiche les ${cartes} kits`);
...
ok(apres.length === 5, 'aucun kit dupliqué par la modification');
...
await page.waitForTimeout(1000);
ok((await lireKits()).length === 6, 'un kit créé s’ajoute à la liste');
...
ok(dupli.length === 7 && dupli.some((k) => k.name === 'Kit 1 kWh — Mini (copie)'),
...
await page.waitForTimeout(1000);
ok((await lireKits()).length === 6, 'la suppression retire bien le kit');
