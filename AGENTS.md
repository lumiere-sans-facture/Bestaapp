...
Contexte pour un agent travaillant sur **BestaSolar Pro** : CRM solaire local-first
...
> Ce fichier miroir volontairement `CLAUDE.md` (mêmes règles, pour Claude Code) — les deux
> doivent rester synchronisés. Pour la carte complète du système (flux de données, schéma
> BDD, paliers de montée en charge), voir **`ARCHITECTURE.md`**.
...
7. **Toute nouvelle collection d'état** (comme `kits` ou `inverters`) suit le même
   patron : dotation dans `dataState.js` (`buildInitialState` + migration dans
   `loadState`), actions dans `context/actions/<domaine>.js`, ajout à
   `SYNCED_COLLECTIONS` (`lib/remoteSync.js`) et table jumelle dans `supabase/schema.sql`
   **et** `supabase/multitenant.sql` (même forme `id text primary key, data jsonb`).
...
- `src/context/` : état global (Auth, Data, Mode, Cart). `DataContext` est un
  *composition root* mince qui assemble `dataState.js` (forme/migration/persistance),
  ...
  leads, partners, catalogue, kits, inverters, devis, formations, pro) et
  `useRemoteSync.js` (moteur de réplication). Ajouter une action → le module de domaine
  concerné dans `context/actions/`, pas le Provider.
- `src/lib/` : accès Supabase (client + réplication).
- `src/data/` : données de référence (seed, catalogue, kits, onduleurs, ensoleillement).
...
- **Constantes plutôt que littéraux dupliqués** : taux TVA → `config/company.js`
  ...
  `DataContext` (`COMMISSION_RATES`), marge prix public → `utils/price.js`
  (`PUBLIC_MARKUP`, via `prixPublic()`). Réutiliser, ne pas recopier une valeur.
- **Prix public partout où un devis ou la Boutique affiche un prix issu du
  catalogue** (`product.basePrice`) : toujours passer par `prixPublic()`
  (`utils/price.js`) — jamais le prix technicien brut sur un document client, peu
  importe le rôle de qui le crée.
...
## Git / déploiement — travail en parallèle avec Claude Code

- **Ne pas développer sur une branche déjà utilisée par Claude Code** (ex. celle en
  cours dans `CLAUDE.md` ou visible via `git branch -a`) : deux agents qui poussent sur
  la même branche en parallèle se marchent dessus (conflits fréquents, historique
  emmêlé). Créer sa propre branche (`codex/…`) à partir du dernier état poussé, et
  fusionner par revue explicite quand le travail est stable — pas de push continu
  croisé sur une branche partagée.
- Avant de commencer, `git fetch` + regarder les branches et commits récents pour
  éviter de refaire un travail déjà en cours côté Claude Code (voir `HISTORY.md` pour
  le journal des changements récents, par session).
