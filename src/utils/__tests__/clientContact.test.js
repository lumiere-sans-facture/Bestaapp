...
describe('identité et traçabilité des clients', () => {
  it('reconnaît le même téléphone malgré la mise en forme et accepte un e-mail seul', () => {
    // Le pays par défaut est le TOGO (shared/phone.js) : un numéro sans
    // indicatif y est togolais. Comparer « +229 … » à « 0161732956 » revenait
    // donc à comparer un béninois à un togolais — le test tenait sur l'ancien
    // défaut Bénin, pas sur l'intention, qui est de reconnaître un même numéro
    // écrit de deux façons.
    expect(isSameClient({ phone: '+228 90 12 34 56' }, { phone: '90 12 34 56' })).toBe(true);
    // Et un numéro béninois reste reconnu des deux côtés, y compris entre
    // l'ancien plan à 8 chiffres et le nouveau à 10.
    expect(isSameClient({ phone: '+229 61 73 29 56' }, { phone: '+2290161732956' })).toBe(true);
    // Deux pays différents ne sont jamais le même client.
    expect(isSameClient({ phone: '+228 90 12 34 56' }, { phone: '+229 90 12 34 56' })).toBe(false);
